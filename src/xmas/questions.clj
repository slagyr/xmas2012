...
(def questions
  [
    ...
    {:icon :snowman :a "Mark Grant" :q "I was nearly kidnapped by gypsies in Paris and I have dreams (nighmares?) where I'm a PHP programmer."}
...
(defn all-icons []
  (sort-by name (map :icon questions)))

(defn generate-answers-html []
  (html
    (html5 {}
      [:head (include-css "../resources/xmas.css")]
      [:body {}
       [:div#main {}
        (for [entry (apply interleave (partition 25 (sort-by :icon questions)))]
          [:div.answer
           [:img {:src (format "../resources/images/%s.png" (name (:icon entry)))}]
           [:p [:em (title-case (name (:icon entry))) ":"] (:a entry)]])]])))

(defn render-answers []
  (let [html (generate-answers-html)
        outfile (file "output/answers.html")]
    (spit outfile html)))
